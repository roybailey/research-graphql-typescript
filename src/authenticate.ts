...
const jwtStategy = new Strategy(
    ...
    (req: Request, jwtPayload: JwtPayload, done: VerifiedCallback) => {
        ...
        getUserByUsername(jwtPayload.userId)
            .then((user) => {
                if (user) {
 ...
 * It will use the UserModel to check for user and password
...
export function setupPassportAuth(app: Application, addDebugRoutes = false) {
    ...
    app.post('/login', async (req, res, next) => {
        try {
            ...
            // Check user password using custom method in UserModel Schema methods
